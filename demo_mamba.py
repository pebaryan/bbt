#!/usr/bin/env python3
"""Generate text from a trained Mamba checkpoint."""
...
def load_model(ckpt_path: str, device: torch.device) -> tuple[MambaMLM, dict]:
    ...
    model = MambaMLM(
        ...
        d_model=_ckpt_arg(cfg, "d_model", 384),
        d_ff=_ckpt_arg(cfg, "d_ff", 768),
        d_state=_ckpt_arg(cfg, "d_state", 16),
...
def main() -> None:
    ...
    ap.add_argument(
        "--ckpt", type=str, default="artifacts/checkpoints/mamba/ckpt_mamba.pt"
    )
    ...
    ap.add_argument(
        "--metrics",
        action="store_true",
        help="Print performance metrics after generation",
    )
    ap.add_argument(
        "--metrics_json",
        type=str,
